...
Focus on stocks that wick below CRT Low and bounce back
"""
...
def show_wick_below_filter(wick_below_stocks):
    """
    Display dynamic filtering for Wick Below stocks
    Focus on bounce strength and CRT velocity
    """
    
    if wick_below_stocks.empty:
        st.warning("No Wick Below patterns available for filtering")
        return wick_below_stocks
    ...
    filtered_stocks = wick_below_stocks.copy()
    ...
    with col1:
        ...
        velocities = wick_below_stocks['CRT_Velocity']
        ...
        selected_percentile = st.radio(
            ...
            key="wick_below_percentile_radio"
        )
    ...
    with col2:
        st.markdown("**Bounce Strength Filter:**")
        
        # Calculate bounce percentage (how far it bounced from low to close)
        bounce_pct = ((wick_below_stocks['Close'] - wick_below_stocks['Low']) / 
                     (wick_below_stocks['CRT_Low'] - wick_below_stocks['Low']) * 100)
        
        bounce_options = {
            "Strong Bounce (>75%)": 75,
            "Moderate Bounce (>50%)": 50,
            "Any Bounce": 0,
            "No Filter": None
        }
        
        selected_bounce = st.radio(
            "Select bounce strength:",
            list(bounce_options.keys()),
            key="wick_below_bounce_radio"
        )
        
        # Apply bounce filtering
        if selected_bounce != "No Filter":
            bounce_threshold = bounce_options[selected_bounce]
            mask = bounce_pct >= bounce_threshold
            filtered_stocks = filtered_stocks[mask.loc[filtered_stocks.index]]
            st.info(f"Bounce strength ≥ {bounce_threshold}%")
        else:
            st.info("All bounce strengths included")
        
        # Show bounce statistics
        with st.expander("Bounce Strength Statistics", expanded=False):
            bounce_stats = {
                "Metric": ["Count", "Min", "25th %ile", "Median", "75th %ile", "Max"],
                "Value": [
                    len(bounce_pct),
                    f"{bounce_pct.min():.1f}%",
                    f"{np.percentile(bounce_pct, 25):.1f}%",
                    f"{bounce_pct.median():.1f}%",
                    f"{np.percentile(bounce_pct, 75):.1f}%",
                    f"{bounce_pct.max():.1f}%"
                ]
            }
            st.dataframe(pd.DataFrame(bounce_stats), hide_index=True, use_container_width=True)
    ...
    if selected_bounce != "No Filter":
        filter_summary.append(f"{selected_bounce}")
    ...
    with st.expander("📊 Distribution Analysis", expanded=False):
        ...
        with col2:
            # Bounce strength distribution
            fig_bounce = px.histogram(
                x=bounce_pct,
                nbins=20,
                title="Bounce Strength Distribution (%)",
                labels={'x': 'Bounce Strength (%)', 'y': 'Count'}
            )
            if selected_bounce != "No Filter":
                bounce_threshold = bounce_options[selected_bounce]
                fig_bounce.add_vline(x=bounce_threshold, line_dash="dash", line_color="red",
                                   annotation_text=f"{selected_bounce} threshold")
    ...
    if len(filtered_stocks) > 0:
        ...
        # Add bounce strength column
        filtered_stocks_sorted['Bounce_%'] = ((filtered_stocks_sorted['Close'] - filtered_stocks_sorted['Low']) / 
                                             (filtered_stocks_sorted['CRT_Low'] - filtered_stocks_sorted['Low']) * 100)
        
        # Display columns focused on wick below analysis
        display_cols = ['Ticker', 'Name', 'Low', 'CRT_Low', 'Close', 'Bounce_%', 
                       'CRT_Velocity', 'IBS', 'Valid_CRT']
        ...
        column_config = {
            ...
            'Bounce_%': st.column_config.NumberColumn('Bounce %', format='%.1f%'),
        ...
        st.text_area(
            f"Singapore Exchange (SGX) Wick Below Patterns ({len(tv_tickers)} stocks):",
            value=tv_string,
            height=100,
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"wick_below_patterns_{len(filtered_stocks)}_stocks.csv",
            mime="text/csv"
        )
...
def show():
    """Main scanner page display for Wick Below patterns"""
    
    st.title("📊 CRT Wick Below Scanner")
    st.markdown("Focus on stocks that test and bounce off CRT Low levels")
    ...
    try:
        ...
        for i, (ticker, df_raw) in enumerate(stock_data.items()):
            try:
                ...
                # Collect results (focused on Wick Below)
                try:
                    result = {
                        'Ticker': ticker,
                        'Name': company_name,
...
def display_scan_results(results_df: pd.DataFrame):
    """Display scanning results focused on Wick Below patterns"""
    ...
    try:
        ...
        wick_below_count = len(results_df[results_df['Wick_Below'] == 1])
        valid_crt_count = len(results_df[results_df['Valid_CRT'] == 1])
        wick_below_with_crt = len(results_df[(results_df['Wick_Below'] == 1) & (results_df['Valid_CRT'] == 1)])
        ...
        with col2:
            st.metric("Wick Below Patterns", wick_below_count, delta=f"{wick_below_count/total_stocks*100:.1f}%")
        with col3:
            st.metric("Valid CRT", valid_crt_count)
        with col4:
            st.metric("Wick Below + CRT", wick_below_with_crt)
        ...
        # Wick Below Patterns Section
        st.subheader("📊 Wick Below Patterns Detected")
        
        wick_below_df = results_df[results_df['Wick_Below'] == 1].copy()
        
        if len(wick_below_df) > 0:
            # Show stocks with both Wick Below AND Valid CRT first
            wick_below_with_crt_df = wick_below_df[wick_below_df['Valid_CRT'] == 1]
            
            if len(wick_below_with_crt_df) > 0:
                st.success(f"🎯 {len(wick_below_with_crt_df)} stocks with Wick Below + Valid CRT")
                show_wick_below_filter(wick_below_with_crt_df)
            else:
                st.info("📊 Wick Below patterns found but none with Valid CRT")
                # Still show the filtering interface for all Wick Below patterns
                show_wick_below_filter(wick_below_df)
        else:
            st.info("🔍 No Wick Below patterns detected in this analysis.")
        ...
        if len(valid_crt_stocks) > 0:
            ...
            with st.expander("📊 Full Valid CRT List (Click to expand)", expanded=False):
                display_cols = ['Ticker', 'Name', 'Analysis_Date', 'Weekly_Open', 'CRT_Velocity', 
                              'CRT_High', 'CRT_Low', 'Wick_Below', 'IBS']
        ...
        with st.expander("📋 Full Analysis Results", expanded=False):
            full_results_cols = [
                'Analysis_Date', 'Ticker', 'Name', 'Low', 'CRT_Low', 'Close', 
                'Wick_Below', 'Valid_CRT', 'CRT_Velocity', 'VW_Range_Percentile', 
        ...
        error_logger.log_debug("Results Display", "Successfully displayed all results", {
            ...
            "wick_below_displayed": len(wick_below_df) if 'wick_below_df' in locals() else 0
        })
